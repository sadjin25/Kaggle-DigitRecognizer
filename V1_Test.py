...
class Train_DigitDataset(Dataset):
    def __init__(self, csv_file):
        ...
        pixels = df.drop(columns=["label"]).to_numpy()
        # (num_samples, x, y)
        self.imgs = torch.tensor(pixels.reshape(-1, 28, 28).transpose(0, 2, 1), 
                                 dtype=torch.float32) 
    def __len__(self):
...
class DigitRecongnizerModel(nn.Module):
    def __init__(self, in_channels=1, out_channels=10, pool_size = 2):
        super().__init__()
        ...
        self.fc1 = nn.Linear(out_channels*(28//self.pool_size**2)**2, 120) # if outchannel = 8 && poolsize = 2, Image size is 32x32, res has 2 pools(2,2).
